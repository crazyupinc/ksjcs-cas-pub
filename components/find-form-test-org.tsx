...
const FindFormTestOrg: NextPage<FindFormTestOrgType> = ({ className = "" }) => {
  return (
    <div
      className={`w-[492px] bg-text-gray max-w-full overflow-hidden flex flex-col items-center justify-center p-2.5 box-border text-left text-sm text-text-text-small-light font-body-text-smaller-text ${className}`}
    >
      <div className="w-full rounded-xl bg-bg-white box-border flex flex-col items-center justify-start p-9 gap-[40px] min-w-[320px] max-w-[600px] border-[1px] border-solid border-colors-neutral-300 mq420:pl-4 mq420:pr-4 mq420:box-border">
        ...
        <form className="m-0 self-stretch flex flex-col items-center justify-start gap-[24px]">
          ...
          <button className="cursor-pointer [border:none] py-3.5 px-7 bg-bg-primary self-stretch rounded-md overflow-hidden flex flex-row items-center justify-center hover:bg-royalblue mq420:py-2.5 mq420:px-4 mq420:box-border">
            <div className="relative text-base leading-[24px] font-body-text-smaller-text text-bg-white text-center mq420:text-smi">
              아이디 찾기
            </div>
          </button>
        </form>
